...
```{python}
# Drop stalk_root column and original target column "class" because numeric column of target values "is_poisonous" added to replace it
...
We summarize all 21 categorical predictors in Table @tbl-feature-summary, which reports the number of levels and the dominant category for each feature.
...
exclude = ["veil_type", "is_poisonous"]  # add more if needed
...
#removing veil_type column as we found in EDA that all values are identical and there is no info to learn
...
# combine training features and targets for analysis of training data

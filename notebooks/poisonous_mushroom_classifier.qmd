...
```{python}
# Data Validation Check: Correct Column Names
...
expected_col_names = [col for col in dataset_col_names if col not in ['stalk_root', 'class']] + ['is_poisonous']
actual_col_names = list(df.columns)
assert expected_col_names == actual_col_names, "The dataframe does not have the expected column names"
```
...
```{python}
X_train.drop('veil_type', axis=1, inplace=True)
X_test.drop('veil_type', axis=1, inplace=True)

...
![](../results/figures/stacked_popluation.png)
...
To explore how individual mushroom characteristics relate to toxicity, we compared the proportion of poisonous and edible mushrooms across several categorical features. 

<!-- `{python} odor_strength`  this is throwing an error due to the table import so the table doesn't show in html rendered-->
...
# odor_strength = poison_rank["feature"].iloc[0]
...
cramers_v_matrix = pd.read_csv("../results/tables/cramers_v_matrix.csv")
Markdown(cramers_v_matrix.to_markdown(index = False))
```

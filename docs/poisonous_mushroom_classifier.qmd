...
The Agaricus and Lepiota (Agaricaceae) family of mushrooms, includes many of the common gilled mushrooms found and cultivated in urban and suburban North America [@mushbook]. 
The family includes the common cultivated mushroom (Agaricus bisporus), and some of the best edible mushrooms, but it also includes poisonous mushrooms and a few that are deadly [@mushbook]. 
These mushrooms are sometimes found in forests, but many grow on lawns, in the compost, or along roadsides and grassy areas where they are readily accessible for human consumption [@mushbook]. 
Accurate assessment of the edibility of these mushrooms is extremely important prior to consumption, however there is no simple rule for determining whether any example is poisonous or edible [@mushbook]. 
...
Answering this question is important because mushroom poisonings are common and the greatest risk factor for mushroom poisoning is the misidentification of poisonous species as edible, in particular by amateur mushroom foragers and immigrants [@poisonarticle]
...
The data set used in this project includes hypothetical mushroom samples drawn from The Audobon Society Field Guide to North American Mushrooms by Gary Lincoff [@mushbook]. 
...
It includes 8124 descriptions of hypothetical mushroom samples that correspond to 23 species of mushrooms in the Agaricus and Lepiota Family [@datasource]. 
...
mushroom_feature_summary = pd.read_csv("../results/mushroom_feature_summary.csv")
Markdown(mushroom_feature_summary.to_markdown(index = False))
```

We summarize all 21 categorical predictors in @tbl-feature-summary, which reports the number of levels and the dominant category for each feature. Given that veil_type only has one level, the values are the same for all observations so we drop that column. 
...
odor_strength = poison_rank["feature"].values[0]
...
It is also worth noting that the hypothetical mushroom observations in this model are based upon a book from 1981 [@mushbook], so it would be wise to consider whether the data could be updated to be sourced from a more recent source of mushroom science. 

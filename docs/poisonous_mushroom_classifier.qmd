...
veil_type = col_drop["feature"].iloc[1]
```

We summarize all 21 categorical predictors in @tbl-feature-summary, which reports the number of levels and the dominant category for each feature. Given that `{python} veil_type` only has one level, the values are the same for all observations so we drop that column. 
...
`{python} veil_type` was dropped because it contains only a single category for all observations.
